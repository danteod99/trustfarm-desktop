<template>
  <div class="relative" @contextmenu.prevent="showContextMenu($event)">
        ...
        <div
          ...
          <div class="relative flex-1 object-fill" :style="'width:' + width + 'px;height:' + height + 'px'">
            <canvas
              ...
              @mousemove="mouseMoveListener" @contextmenu.prevent="showContextMenu($event)" tabindex="0" @keydown="keyDownListener" @keyup="keyUpListener"></canvas>

            <!-- Context menu is teleported to body -->

            <div @click="$emiter('openDevice', this.device)" @contextmenu.prevent="showContextMenu($event)"
  ...
  <Teleport to="body">
    <div v-if="contextMenuVisible"
      class="fixed z-[9999] bg-base-100 border border-base-300 rounded-lg shadow-2xl py-1 min-w-48"
      :style="{ left: contextMenuX + 'px', top: contextMenuY + 'px' }"
      @click.stop>
      <div class="text-xs font-bold text-base-content/50 uppercase px-3 py-1">Navigation</div>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'input', 'keyevent', '3')">
        <font-awesome-icon icon="fa fa-home" class="w-3 h-3" /> Home
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'input', 'keyevent', '4')">
        <font-awesome-icon icon="fa fa-arrow-left" class="w-3 h-3" /> Back
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'input', 'keyevent', '187')">
        <font-awesome-icon icon="fa fa-layer-group" class="w-3 h-3" /> Recent Apps
      </button>
      <div class="divider my-0.5 px-2"></div>
      <div class="text-xs font-bold text-base-content/50 uppercase px-3 py-1">Open App</div>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'am', 'start', '-n', 'com.instagram.android/com.instagram.mainactivity.InstagramMainActivity')">
        <font-awesome-icon icon="fa-brands fa-instagram" class="w-3 h-3 text-pink-500" /> Instagram
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'am', 'start', '-n', 'com.facebook.katana/com.facebook.katana.LoginActivity')">
        <font-awesome-icon icon="fa-brands fa-facebook" class="w-3 h-3 text-blue-500" /> Facebook
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'am', 'start', '-n', 'com.zhiliaoapp.musically/com.ss.android.ugc.aweme.splash.SplashActivity')">
        <font-awesome-icon icon="fa-brands fa-tiktok" class="w-3 h-3" /> TikTok
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'am', 'start', '-n', 'com.android.chrome/com.google.android.apps.chrome.Main')">
        <font-awesome-icon icon="fa-brands fa-chrome" class="w-3 h-3 text-green-500" /> Chrome
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'am', 'start', '-a', 'android.intent.action.MAIN', '-c', 'android.intent.category.APP_GALLERY')">
        <font-awesome-icon icon="fa fa-images" class="w-3 h-3 text-purple-500" /> Gallery
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'am', 'start', '-a', 'android.settings.SETTINGS')">
        <font-awesome-icon icon="fa fa-cog" class="w-3 h-3" /> Settings
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'am', 'start', '-a', 'android.settings.WIFI_SETTINGS')">
        <font-awesome-icon icon="fa fa-wifi" class="w-3 h-3 text-blue-400" /> WiFi Settings
      </button>
      <div class="divider my-0.5 px-2"></div>
      <div class="text-xs font-bold text-base-content/50 uppercase px-3 py-1">Device</div>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxInstallApk">
        <font-awesome-icon icon="fa fa-download" class="w-3 h-3 text-info" /> Install APK
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'screencap', '-p', '/sdcard/screenshot.png')">
        <font-awesome-icon icon="fa fa-camera" class="w-3 h-3" /> Screenshot
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-primary hover:text-primary-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'input', 'keyevent', '26')">
        <font-awesome-icon icon="fa fa-power-off" class="w-3 h-3" /> Power Button
      </button>
      <button class="w-full text-left px-3 py-1.5 hover:bg-error hover:text-error-content flex items-center gap-2 text-sm" @click="ctxAction('shell', 'reboot')">
        <font-awesome-icon icon="fa fa-redo" class="w-3 h-3" /> Reboot
      </button>
    </div>
  </Teleport>
...
export default {
  ...
  methods: {
    ...
    async ctxInstallApk() {
      ...
      if (filePath) {
        await this.$emiter('adbEventData', { args: ['install', '-r', filePath] });
        await this.$emiter('NOTIFY', { type: 'success', message: 'Installing APK...', timeout: 3000 });
      }
    },
    async touchSync(operation, event) {
      if (!this.big && !this.operating) {
        return
      }
    ...
    mouseDownListener(event) {
      ...
      // 让canvas获得焦点以接收键盘事件
      if ((this.big || this.operating) && this.$refs.canvas) {
