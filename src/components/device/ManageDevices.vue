<template>
    ...
    <div
      ...
      <div class="h-full overflow-y-auto no-scrollbar px-4 pb-20 pt-4 space-y-4">
        ...
        <Pagination ref="device_panel" :items="mydevices" :pageSize="200" @refresh="refreshPage" :showTopControls="true"
          ...
          <template #buttons>
            <div class="flex flex-wrap items-center justify-between gap-2 w-full px-1">
              <div class="flex flex-wrap items-center gap-1">
                <button class="btn btn-sm btn-primary" @click="$refs.scan_dialog.show()">
                  <font-awesome-icon icon="fa-solid fa-network-wired" class="h-3 w-3" />{{ $t('scanTCPDevice') }}
                </button>
                <button class="btn btn-sm btn-primary" @click="$refs.accountManager.show()">
                  <font-awesome-icon icon="user" class="h-3 w-3" />{{ $t('accounts') }}
                </button>
                <button class="btn btn-sm btn-primary"
                  @click="$emiter('showDialog', { name: 'tasks' })">
                  <font-awesome-icon icon="fa-solid fa-list-check" class="h-3 w-3" />Tasks
                </button>
              </div>

              <div class="flex flex-wrap items-center gap-1">
                <!-- Screen Cast Settings Button -->
                <button class="btn btn-sm btn-circle btn-ghost tooltip tooltip-bottom"
                ...
                <div
                  class="flex items-center gap-1 px-2 py-1 rounded-lg bg-base-200/80 border border-base-300/60 shadow-sm"
                  ...
                  <div
                    ...
                    <button type="button"
                      class="join-item btn btn-sm gap-1 px-2 py-1 border-0 bg-transparent hover:bg-primary/10"
  ...
  <AccountManager ref="accountManager" :devices="mydevices" />
</template>
<style>
...
export default {
  ...
  async created() {
    ...
    try {
      this.bigScreenMode = (storedBigScreen && String(storedBigScreen).replace(/"/g, '')) || 'docked'
    } catch (err) {
      this.bigScreenMode = 'docked'
    }
    try {
