...
import { getItem, setItem, getJsonItem } from '@/utils/storage.js';
...
export default {
    ...
    async loadLicense() {
      ...
      try {
        ...
        if (res.code === 0) {
          ...
          // Determine plan based on user login status
          const user = await getJsonItem('trustmind_user');
          if (user && user.email) {
            // Pro plan for logged-in users
            this.licenseData.plan_name = 'TrustFarm Pro';
            this.licenseData.leftdays = 9999;
            this.licenseData.is_stripe_active = 1;
            this.licenseData.concurrency_limit = 100;
            this.licenseData.features = ['whiteLabel', 'followPlan', 'superMarketing', 'aiAgent', 'massDM', 'massComment'];
          } else {
            // Free plan for anonymous users
            this.licenseData.plan_name = 'TrustFarm Free';
            this.licenseData.leftdays = 9999;
            this.licenseData.is_stripe_active = 0;
            this.licenseData.concurrency_limit = 2;
            this.licenseData.features = ['warmup', 'login'];
          }
  ...
  async mounted() {
    ...
    // Reload license when user logs in/out
    await this.$listen('USER_AUTH_CHANGED', async () => {
