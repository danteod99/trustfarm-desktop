<template>
  <div ref="sidebarContainer"
    ...
    <div class="flex-1 overflow-y-auto no-scrollbar px-2 pt-3 pb-4 space-y-4">
      ...
      <div class="tabs tabs-md tabs-border border border-base-300 bg-base-500 rounded-md shadow-lg p-2">
        ...
        <input type="radio" name="my_tabs_3" class="tab" aria-label="Automations" />
        <div class="tab-content mt-2">
    ...
    <div class="shrink-0 bg-base-100 border-t border-base-200/70 px-3 py-2">
      <div v-if="currentUser" class="flex items-center gap-2">
        <div class="w-8 h-8 rounded-full bg-primary text-primary-content flex items-center justify-center text-sm font-bold shrink-0">
          {{ userInitial }}
        </div>
  ...
  <dialog ref="loginDialog" class="modal">
    <div class="modal-box max-w-sm">
      <h3 class="font-bold text-lg mb-4">{{ $t('signIn') || 'Sign in' }}</h3>

      <div v-if="loginStep === 'start'" class="space-y-3">
        <p class="text-sm text-base-content/70">{{ $t('signInDescription') || 'Sign in with your TrustMind account to sync your license and settings.' }}</p>
        <button @click="openLoginBrowser" class="btn btn-primary w-full gap-2">
          <font-awesome-icon icon="fa-solid fa-arrow-up-right-from-square" class="h-4 w-4" />
          {{ $t('signInWithGoogle') || 'Sign in with Google' }}
        </button>
      </div>

      <div v-else-if="loginStep === 'paste'" class="space-y-3">
        <p class="text-sm text-base-content/70">{{ $t('pasteTokenDescription') || 'After signing in, copy the code and paste it here:' }}</p>
        <textarea v-model="loginToken" class="textarea textarea-bordered w-full h-24 text-xs font-mono"
          :placeholder="$t('pasteTokenPlaceholder') || 'Paste your code here...'"
          @keyup.enter="submitLoginToken"></textarea>
        <div v-if="loginError" class="text-error text-sm">{{ loginError }}</div>
        <div class="flex gap-2">
          <button @click="loginStep = 'start'" class="btn btn-ghost btn-sm flex-1">{{ $t('back') || 'Back' }}</button>
          <button @click="submitLoginToken" class="btn btn-primary btn-sm flex-1" :disabled="!loginToken.trim()">
            {{ $t('confirm') || 'Confirm' }}
          </button>
        </div>
      </div>
      ...
      <form method="dialog" class="mt-4">
        <button class="btn btn-ghost btn-sm w-full">{{ $t('cancel') || 'Cancel' }}</button>
      </form>
    </div>
...
import { getJsonItem, setJsonItem, removeItem } from '../utils/storage.js';
...
export default {
  ...
  methods: {
    ...
    async submitLoginToken() {
      this.loginError = ''
      try {
        const raw = this.loginToken.trim()
        const decoded = JSON.parse(atob(raw))
        const accessToken = decoded.a
