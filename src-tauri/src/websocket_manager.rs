...
/// Read WebSocket port from wsport.txt file
fn read_ws_port(app_data_dir: &PathBuf) -> Option<u16> {
    let port_file = app_data_dir.join("wsport.txt");
    match std::fs::read_to_string(&port_file) {
        ...
        Err(e) => {
            log::warn!("Failed to read wsport.txt: {}", e);
            None
        }
    }
}
...
/// Connect to agent WebSocket (reads port from wsport.txt)
#[tauri::command]
