...
pub async fn agent_request(
    ...
    // Read agent port from file, fallback to TikMatrix's port file
    ...
    let port = match fs::read_to_string(&port_file) {
        Ok(content) => {
            ...
            if trimmed == "0" || trimmed.is_empty() {
                // Fallback: read from TikMatrix's port file
                let tikmatrix_port_file = app_data_dir.parent()
                    .map(|p| p.join("com.tikmatrix").join("port.txt"));
                match tikmatrix_port_file.and_then(|f| fs::read_to_string(&f).ok()) {
                    Some(tm_content) if tm_content.trim() != "0" && !tm_content.trim().is_empty() => {
                        log::info!("Using TikMatrix agent port: {}", tm_content.trim());
                        tm_content.trim().to_string()
                    }
                    _ => {
