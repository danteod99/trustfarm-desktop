...
pub fn start_agent_process(app_handle: &AppHandle) -> Result<AgentStartResult, String> {
    ...
    if status.running {
        ...
        // Accept any process on port 50809 as a valid agent (supports TikMatrix.exe embedded agent)
        log::info!("Agent is already running (process: {})", status.process_name);
    ...
    // If agent binary not found, try TikMatrix installed path as fallback
    let agent_path = if !agent_path.exists() {
        let tikmatrix_path = std::path::PathBuf::from(r"C:\Program Files\TikMatrix\TikMatrix.exe");
        if tikmatrix_path.exists() {
            log::info!("Agent binary not found, using TikMatrix.exe as fallback: {:?}", tikmatrix_path);
            tikmatrix_path
        } else {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        command.creation_flags(0x08000000); // Hide console window
        // In release mode, discard stdout/stderr to prevent broken pipe crashes
        // when the Child handle is dropped. Only pipe in debug mode for log capture.
...
pub async fn initialize_app(
    ...
    if !result.agent_status.running {
        ...
        match start_agent_process(&app_handle) {
            Ok(start_result) => {
                if start_result.success {
                    // Wait for agent to be ready (30s for TikMatrix.exe which takes longer to start)
                    match wait_agent_ready(&app_handle, 30).await {
                        Ok(ready) => {
                            if ready {
                                result.success = true;
        ...
        // Agent is already running (accept any process on port 50809)
        log::info!("Agent already running via process: {}", result.agent_status.process_name);
        result.success = true;
...
pub async fn check_and_process_library_updates(
    ...
    // Use TikMatrix as API name to download libs from niostack server
    let app_name = "TikMatrix".to_string();
